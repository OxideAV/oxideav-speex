...
impl NbDecoder {
    ...
    pub fn decode_frame(&mut self, br: &mut BitReader, out: &mut [f32]) -> Result<()> {
        ...
        // The reference does either `multicomb` (perceptual postfilter,
        // when `lpc_enh_enabled` and `comb_gain > 0`) or a plain copy.
        // We do a plain copy to keep the implementation small; the
        // task says "postfilter is optional but recommended". TODO.
        for i in 0..NB_FRAME_SIZE {
            out[i] = self.exc_buf[EXC_HISTORY - NB_SUBFRAME_SIZE + i];
        }

        // ---- LPC synthesis (per-sub-frame, with LSP interpolation) ---
